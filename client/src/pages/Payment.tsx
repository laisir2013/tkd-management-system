    ...
    <div className="space-y-4">
      ...
      {students.length === 1 ? (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {students.map(student => (
              <div key={student.id} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 cursor-pointer" onClick={() => handleStudentToggle(student.id)}>
                <Checkbox
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {PERIOD_OPTIONS.map(option => {
            ...
            const isDisabled = isLocked || isDisabledByCustom || isDisabledByQuarter;
            return (
              <div key={option.value} className={`flex items-center space-x-3 p-3 rounded-lg border ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'} ${
                isChecked && !isLocked
              ...
              }`} onClick={() => { if (!isDisabled) handlePeriodToggle(periodValue); }}>
                <Checkbox
                  checked={isChecked}
